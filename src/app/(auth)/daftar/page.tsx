...
import { Loader2, AlertCircle, Bot, ShieldCheck, Coins, CheckCircle2 } from 'lucide-react';
import Link from 'next/link';
import { registrasiSiswaBaru } from '@/actions/auth-siswa';
...
export default function DaftarPage() {
  ...
  const tanganiDaftar = async (e: React.FormEvent) => {
    ...
    try {
      const hasil = await registrasiSiswaBaru({
        nama,
        email,
        noTelepon,
        kataSandi
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 overflow-y-auto">
        <div className="w-full max-w-md py-8">
          ...
          <div className="text-center mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900">Buat Akun Siswa Baru</h2>
            <p className="mt-2 text-sm text-slate-600">
              Mulai langkah belajarmu bersama BimbelPro hari ini.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-200">
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white text-slate-500">atau isi formulir pendaftaran</span>
              </div>
            </div>

            <form className="space-y-4" onSubmit={tanganiDaftar}>
              {error && (
              ...
              {sukses && (
                <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex items-start space-x-3 text-emerald-700">
                  <CheckCircle2 size={20} className="shrink-0 mt-0.5" />
                  <span className="text-sm font-medium">{sukses}</span>
                </div>
              )}
              ...
              <div>
                <label htmlFor="noTelepon" className="block text-sm font-medium text-slate-700 mb-1">
                  No. WhatsApp
                </label>
                <input
                  id="noTelepon"
          ...
          <p className="mt-8 text-center text-sm text-slate-600">
            Sudah punya akun?{' '}
            <Link href="/masuk" className="font-semibold text-indigo-600 hover:text-indigo-500 transition-colors">
              Masuk di sini
            </Link>
          </p>
        </div>
