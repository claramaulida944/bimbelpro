...
const navigasiMenu = [
  { nama: 'Beranda', rute: '/admin', ikon: LayoutDashboard },
  { nama: 'Data Siswa', rute: '/admin/siswa', ikon: Users },
  { nama: 'Kursus & Materi', rute: '/admin/kursus', ikon: BookOpen },
  { nama: 'Simulasi Ujian', rute: '/admin/ujian', ikon: FileQuestion },
  { nama: 'Keuangan', rute: '/admin/keuangan', ikon: CreditCard },
  { nama: 'AI Studio', rute: '/admin/ai-generator', ikon: Bot },
  { nama: 'Broadcast WA', rute: '/admin/whatsapp', ikon: MessageSquare },
  { nama: 'Pengaturan', rute: '/admin/pengaturan', ikon: Settings },
];
...
export default function SidebarAdmin() {
  ...
  const namaPengguna = session?.user?.name || 'Administrator';
  const emailPengguna = session?.user?.email || '-';
  const inisialPengguna = ambilInisial(namaPengguna);
  const fotoProfil = session?.user?.image;
  // Peran bisa di-derive atau default
  const peranTeks = session?.user?.peran === 'ADMIN_PENGAJAR' ? 'ADMIN PENGAJAR' : 'ADMINISTRATOR';
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-2 no-scrollbar">
          {navigasiMenu.map((item) => {
            ...
            const aktif = pathname === item.rute || (pathname.startsWith(`${item.rute}/`) && item.rute !== '/admin');
          ...
          {popoverBuka && (
            ...
            >
              <div className="p-4 border-b border-slate-100">
                ...
                <span className="inline-block mt-2 px-2.5 py-0.5 bg-indigo-50 text-indigo-700 text-[10px] font-bold rounded-md border border-indigo-100 uppercase tracking-wider">
                  {peranTeks}
                </span>
              </div>
            ...
            <div className="flex items-center gap-3">
              {fotoProfil ? (
                <img 
                  src={fotoProfil} 
                  alt={namaPengguna} 
                  className="w-10 h-10 rounded-full object-cover border border-indigo-200 flex-shrink-0"
                  referrerPolicy="no-referrer"
                />
              ) : (
                <div className="w-10 h-10 rounded-full bg-indigo-100 text-indigo-700 font-bold border border-indigo-200 flex items-center justify-center flex-shrink-0">
                  {inisialPengguna}
                </div>
              )}
              <div 
